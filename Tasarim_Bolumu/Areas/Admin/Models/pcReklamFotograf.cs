...
{
    public class pcReklamFotograf
    {
        public static string FotografYukle(List<IFormFile> files, IWebHostEnvironment _env, out bool result, SqlDbContext context, pcReklam item)
        {
            ...
            result = false;
            var uploads = Path.Combine(_env.WebRootPath, "pcReklamFotograf");
                ...
                if (file.ContentType.Contains("image"))
                {
                    if (file.Length <= 2097152)
                    {
                        ...
                        using (var fileStream = new FileStream(filePath, FileMode.Create))
                        {
                            ...
                            var field = context.pcReklam.Find(item.Id);

                            var yeni = field.fotografAdiPcReklam.Substring(1);
                            ...
                            return filePath.Substring(filePath.IndexOf("\\pcReklamFotograf\\"));
                        }
                    }
                    else
                    {
