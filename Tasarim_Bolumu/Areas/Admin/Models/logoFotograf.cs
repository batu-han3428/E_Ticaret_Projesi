...
{
    public class logoFotograf
    {
        public static string FotografYukle(List<IFormFile> files, IWebHostEnvironment _env, out bool result, SqlDbContext context, siteLogo item)
        {
            ...
            result = false;
            var uploads = Path.Combine(_env.WebRootPath, "siteLogoFotograf");
                ...
                if (file.ContentType.Contains("image"))
                {
                    if (file.Length <= 2097152)
                    {
                        ...
                        using (var fileStream = new FileStream(filePath, FileMode.Create))
                        {
                            ...
                            var field = context.siteLogo.Find(item.Id);

                            var yeni = field.siteLogoFoto.Substring(1);
                            ...
                            return filePath.Substring(filePath.IndexOf("\\siteLogoFotograf\\"));
                        }
                    }
                    else
                    {
