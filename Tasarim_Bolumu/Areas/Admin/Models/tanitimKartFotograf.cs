...
{
    public class tanitimKartFotograf
    {
        public static string FotografYukle(List<IFormFile> files, IWebHostEnvironment _env, out bool result, SqlDbContext context, TanitimKart item)
        {
            ...
            result = false;
            var uploads = Path.Combine(_env.WebRootPath, "tanitimKartFotograflar");
                ...
                if (file.ContentType.Contains("image"))
                {
                    if (file.Length <= 2097152)
                    {
                        ...
                        using (var fileStream = new FileStream(filePath, FileMode.Create))
                        {
                            ...
                            var field = context.tanitimKart.Find(item.Id);

                            var yeni = field.tanitimKartFotografAdi.Substring(1);
                            ...
                            return filePath.Substring(filePath.IndexOf("\\tanitimKartFotograflar\\"));
                        }
                    }
                    else
                    {
